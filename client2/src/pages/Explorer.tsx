...
const ExplorerDetail: React.FC<ExplorerDetailProps> = ({ cryptocurrencyId }) => {
  ...
  // Get basic data
  const { data: basicData, isLoading: isLoadingBasicData } = useQuery({
    queryKey: [`/api/cryptocurrencies/${cryptocurrencyId}/basic-data`],
    enabled: !!cryptocurrencyId,
  });
        ...
        <>
          ...
          <Tabs defaultValue="overview">
            <TabsList className="mb-4">
              ...
              <TabsTrigger value="basic-data">Basic Data</TabsTrigger>
                ...
                <CardContent>
                  {isLoadingBasicData ? (
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {[
                            { label: '7 Days', value: basicData.priceChange7d, suffix: '%' },
                            { label: '30 Days', value: basicData.priceChange30d, suffix: '%' },
                            { label: '60 Days', value: basicData.priceChange60d, suffix: '%' },
                            { label: '90 Days', value: basicData.priceChange90d, suffix: '%' },
                            { label: '180 Days', value: basicData.priceChange180d, suffix: '%' },
                            { label: '1 Year', value: basicData.priceChange1y, suffix: '%' }
                          ].map((item) => (
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {[
                            { label: 'Circulating Supply', value: basicData.circulatingSupply, format: 'number' },
                            { label: 'Total Supply', value: basicData.totalSupply, format: 'number' },
                            { label: 'Circulating/Total Ratio', value: basicData.circulatingToTotalRatio, format: 'percentage' },
                            { label: 'Volume/Market Cap Ratio', value: basicData.volumeToMarketCapRatio, format: 'ratio' }
                          ].map((item) => (
                            <div key={item.label} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                              <div className="text-sm text-slate-500 dark:text-slate-400">{item.label}</div>
                              <div className="text-lg font-semibold">
                      ...
                      {/* 链上活动数据 */}
                      <div>
                        <h3 className="text-lg font-semibold mb-4">On-Chain Activity</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {[
                            { label: 'Daily Active Addresses', value: basicData.dailyActiveAddresses, format: 'number' },
                            { label: 'Daily Transactions', value: basicData.dailyTransactions, format: 'number' },
                            { label: 'Daily Gas Cost (USD)', value: basicData.dailyGasCost, format: 'currency' },
                            { label: 'Top 10 Address Concentration', value: basicData.top10AddressConcentration, format: 'percentage' }
                          ].map((item) => (
                            <div key={item.label} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                              <div className="text-sm text-slate-500 dark:text-slate-400">{item.label}</div>
                              <div className="text-lg font-semibold">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {[
                            { label: 'Monthly Commits', value: basicData.monthlyCommits, format: 'number' },
                            { label: 'Developer Count', value: basicData.developerCount, format: 'number' },
                            { label: 'Dependent Projects', value: basicData.dependentProjects, format: 'number' },
                            { label: 'Developer Forum Activity', value: basicData.developerForumActivity, format: 'number' }
                          ].map((item) => (
                            <div key={item.label} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                              <div className="text-sm text-slate-500 dark:text-slate-400">{item.label}</div>
                              <div className="text-lg font-semibold">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {[
                            { label: 'Twitter Engagement Rate', value: basicData.twitterEngagementRate, format: 'percentage' },
                            { label: 'Discord/Telegram Activity', value: basicData.discordTelegramActivity, format: 'number' }
                          ].map((item) => (
                            <div key={item.label} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                              <div className="text-sm text-slate-500 dark:text-slate-400">{item.label}</div>
                              <div className="text-lg font-semibold">
                      ...
                      <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                        <div className="text-sm text-slate-500 dark:text-slate-400">
                          <div>Data Source: {basicData.dataSource || 'Unknown'}</div>
                          <div>Last Updated: {basicData.lastUpdated ? formatTimeAgo(basicData.lastUpdated) : 'Unknown'}</div>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center py-8 text-slate-500 dark:text-slate-400">
                      ...
                      <p>No basic data available for this cryptocurrency.</p>
                      <p className="text-sm mt-2">Run the comprehensive data collection to gather detailed metrics.</p>
                    </div>
                  )}
                </CardContent>
              </Card>
